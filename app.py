...
<style>
  body { font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; margin: 0; background: #f0f4f7; color: #333; display: flex; justify-content: center; min-height: 100vh; align-items: flex-start; padding: 20px; }
  #container { width: 100%; max-width: 700px; background: white; border-radius: 10px; box-shadow: 0 0 20px rgb(0 0 0 / 0.15); padding: 30px 35px; }
  header { background: #2e7d32; color: white; display: flex; align-items: center; padding: 15px 20px; border-radius: 10px 10px 0 0; margin: -30px -35px 30px -35px; }
  header img { height: 40px; margin-right: 15px; }
  header h1 { font-size: 1.8rem; margin: 0; flex-grow: 1; }
  a.logout { color: white; text-decoration: none; background: #c62828; padding: 8px 15px; border-radius: 6px; font-weight: bold; }
  a.logout:hover { background: #a62020; }
  section { margin-bottom: 40px; text-align: center; }
  h2 { color: #2e7d32; margin-bottom: 15px; font-weight: 700; font-size: 1.5rem; border-bottom: 2px solid #2e7d32; padding-bottom: 8px; display: inline-block; width: fit-content; }
  .status { font-weight: bold; padding: 12px 20px; border-radius: 8px; display: inline-block; margin-bottom: 15px; font-size: 1.3rem; min-width: 120px; }
  .status.aberto { background: #388e3c; color: white; }
  .status.fechado { background: #c62828; color: white; }
  .temperatura { font-size: 2.5rem; margin: 10px 0 5px; display: flex; justify-content: center; align-items: center; gap: 10px; }
  .temperatura span { font-size: 2.8rem; }
  .hora { color: #666; margin-bottom: 20px; }
  table { width: 100%; border-collapse: collapse; margin-top: 15px; }
  th, td { padding: 10px 12px; border-bottom: 1px solid #ddd; text-align: center; font-size: 1rem; }
  th { background-color: #f2f2f2; }
  button { background-color: #2e7d32; color: white; border: none; padding: 14px 35px; border-radius: 10px; cursor: pointer; font-size: 1.1rem; font-weight: 700; transition: background-color 0.3s ease; }
  button:hover { background-color: #276527; }
  footer { text-align: center; margin-top: 30px; font-size: 0.9rem; color: #aaa; }
</style>
</head>
<body>
  <div id="container">
    ...
    <section id="geladeira">
      ...
      {% if temperatura is not none %}
        <div class="temperatura"><span>🌡️</span> {{ "%.2f"|format(temperatura) }} °C</div>
      ...
      {% if historico %}
        <table>
          <thead><tr><th>Temperatura (°C)</th><th>Hora</th></tr></thead>
          <tbody>
            {% for item in historico %}
              <tr><td>{{ "%.2f"|format(item.temperatura) }}</td><td>{{ item.hora }}</td></tr>
            {% endfor %}
          </tbody>
        </table>
      {% else %}
    ...
    <footer>&copy; 2025 Centro Veterinário Matavelli</footer>
  </div>
</body>
</html>
"""
